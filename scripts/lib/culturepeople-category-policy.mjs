export const CULTUREPEOPLE_CATEGORIES = ["문화", "엔터", "스포츠", "라이프", "테크·모빌리티", "비즈", "공공"];
...
export function isCulturePeopleCategory(value) {
  return CULTUREPEOPLE_CATEGORIES.includes(String(value || "").normalize("NFC").trim());
}

export function classifyCulturePeopleCategory(value, context = "", fallback = "문화") {
  const raw = String(value || "").normalize("NFC").trim();
  if (isCulturePeopleCategory(raw)) return { category: raw, reason: "standard" };
  const alias = CATEGORY_ALIASES[raw] || CATEGORY_ALIASES[raw.toUpperCase()];
  if (alias) return { category: alias, reason: `alias:${raw}` };
  ...
  return { category: best, reason: bestScore > 0 ? `keywords:${bestScore}` : `fallback:${fallback}` };
}
