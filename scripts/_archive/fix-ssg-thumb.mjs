...
async function run() {
  const imgUrl = "https://shinsegae-prd-data.s3.ap-northeast-2.amazonaws.com/wp-content/uploads/2025/12/NR_Press_Details_01-8.png";
  ...
  const path = `2026/03/${Date.now()}_ssg7club_official.png`;
  ...
  const upd = await fetch(`${SB_URL}/rest/v1/articles?no=eq.2992`, {
    ...
    body: JSON.stringify({ thumbnail: newUrl, thumbnail_alt: "SSG닷컴 쓱세븐클럽 멤버십 공식 이미지" }),
  });
  console.log("기사 #2992 썸네일 교체:", upd.ok ? "완료 ✓" : "실패");
}
