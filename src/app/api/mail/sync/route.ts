...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (accounts.length === 0) {
      return NextResponse.json({ success: false, error: "IMAP 계정이 설정되지 않았습니다." }, { status: 400 });
    }
    ...
    for (const account of accounts) {
      const result = await syncAccount(account, days, existingKeys);
      allNewMails.push(...result.newMails);
      allErrors.push(...result.errors);
    }

    // 새 메일 추가 후 저장 (최신순, 최대 MAX_STORED건)
    ...
    return NextResponse.json({ success: false, error: "동기화 중 오류가 발생했습니다." }, { status: 500 });
  }
}
