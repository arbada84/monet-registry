...
const SAMPLE_REPORTERS: Reporter[] = [
  { id: "rpt-1", name: "김문화", email: "kim@culturepeople.co.kr", phone: "010-1234-5678", department: "문화부", title: "부장", photo: "", bio: "문화예술 분야 10년 경력 기자", active: true, articleCount: 120, joinDate: "2024-01-01" },
  { id: "rpt-2", name: "이연예", email: "lee@culturepeople.co.kr", phone: "010-2345-6789", department: "연예부", title: "기자", photo: "", bio: "K-POP, 드라마, 영화 담당", active: true, articleCount: 85, joinDate: "2024-03-15" },
  { id: "rpt-3", name: "박스포츠", email: "park@culturepeople.co.kr", phone: "010-3456-7890", department: "스포츠부", title: "기자", photo: "", bio: "축구, 야구 등 스포츠 전문", active: true, articleCount: 67, joinDate: "2024-05-01" },
];
...
export default function AdminReportersPage() {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("cp-reporters");
    if (stored) {
      setReporters(JSON.parse(stored));
    } else {
      localStorage.setItem("cp-reporters", JSON.stringify(SAMPLE_REPORTERS));
      setReporters(SAMPLE_REPORTERS);
    }
  }, []);

  const saveReporters = (updated: Reporter[]) => {
    setReporters(updated);
    localStorage.setItem("cp-reporters", JSON.stringify(updated));
  };
        ...
        {reporters.map((reporter) => (
          <div key={reporter.id} style={{ background: "#FFF", border: "1px solid #EEE", borderRadius: 10, padding: 20 }}>
            ...
            <div style={{ fontSize: 12, color: "#999", marginBottom: 12 }}>
              {reporter.email} · 기사 {reporter.articleCount}건
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button onClick={() => handleDelete(reporter.id)} style={{ padding: "4px 14px", background: "#FFF", border: "1px solid #E8192C", borderRadius: 6, color: "#E8192C", fontSize: 12, cursor: "pointer" }}>삭제</button>
            </div>
          </div>
        ))}
      </div>
